...
public class CloudSimulationLOA {
	...
	private static List<Vm> createVM(int userId, int vms) {
		...
		for (int i = 0; i < vms; i++) {
			vm[i] = new Vm(i, userId, mips[i % 3], pesNumber, ram[i % 3], bw, size, vmm,
			...
			for (int cloudletIterator = 0; cloudletIterator <= cloudletLoopingNumber; cloudletIterator++) {
				...
				for (int dataCenterIterator = 1; dataCenterIterator <= 6; dataCenterIterator++) {
					...
					for (int assigner = 0 + (dataCenterIterator - 1) * 9 + cloudletIterator * 54; assigner < 9
						...
						broker.bindCloudletToVm(assigner, population.getFittest(dataCenterIterator)
								.getGene(assigner - (dataCenterIterator - 1) * 9 - cloudletIterator * 54));
						outputWriter.write(
								Long.toString(population.getFittest(dataCenterIterator).getGene(assigner % 9) % 9)); // Print
																														// Assigned
																														// VM
																														// ID
																														// %
	...
	private static void printCloudletList(List<Cloudlet> list) throws FileNotFoundException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
				...
				Log.printLine(indent + indent + indent + (cloudlet.getResourceId() - 1) + indent + indent + indent
						+ cloudlet.getVmId() +
