...
import java.util.LinkedList;where
...
public class CloudSimulationLOA {
...
//		Log.printLine(System.getProperty("user.dir")+ "/dataset/RandSimple"+bot+"000.txt");
			...
			for (int cloudletIterator = 0; cloudletIterator <= cloudletLoopingNumber; cloudletIterator++) {
			    ...
			    for (int dataCenterIterator = 1; dataCenterIterator <= 6; dataCenterIterator++) {
			        ...
			        while (iteration <= 15) {
			            ...
			            // Apply lion behavior
			            population = loa.applyLionBehavior(population, dataCenterIterator);
...
//			            broker.bindCloudletToVm(assigner, population.getFittest(dataCenterIterator).getGene(assigner % 9));
			        	broker.bindCloudletToVm(assigner, population.getFittest(dataCenterIterator).getGene(assigner - (dataCenterIterator - 1) * 9 - cloudletIterator * 54));
			            outputWriter.write(Long.toString(population.getFittest(dataCenterIterator).getGene(assigner % 9) % 9)); // Print Assigned VM ID %
			            outputWriter.write(" ");
			        }
